...
class AutoPurchase extends Command
{
    ...
    public function handle()
    {
        ...
        if(count($product_data)) {
            $pos_setting_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('pos_setting')
                                ->select('warehouse_id')
                                ->latest()
                                ->first();
            $user_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('users')
                        ->select('id')
                        ->where([
                            ['is_active', true],
                            ['role_id', 1]
            ...
            foreach($product_data as $key => $product) {
                if($product->tax_id) {
                    $tax_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('taxes')->select('rate')->find($product->tax_id);
