...
class Common
{
    public function handle(Request $request, Closure $next)
    {
        ...
        $alert_product = DB::connection(env('TENANT_DB_CONNECTION'))->table('products')->where('is_active', true)->whereColumn('alert_quantity', '>', 'qty')->count();
        $dso_alert_product = DB::connection(env('TENANT_DB_CONNECTION'))->table('dso_alerts')->select('number_of_products')->whereDate('created_at', date("Y-m-d"))->first();
