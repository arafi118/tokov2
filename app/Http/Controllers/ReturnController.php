...
class ReturnController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $lims_sale_data = Sale::select('id', 'warehouse_id', 'customer_id', 'biller_id', 'is_tempo')->find($data['sale_id']);

            if ($lims_sale_data->is_tempo == 'Ya') {
                $cara_bayar = 'tempo';
            } else {
                $pay = Payment::where('sale_id', $lims_sale_data->id)->first();
            ...
            $data['account_id'] = 0; //$lims_account_data->id;
            ...
            \DB::rollback();
            \Log::error("return sale store : {$e->getMessage()}");
    ...
    private function sendToJournal(array $data)
    {
        $journal_data = [
            ...
            'details' => [
                ...
                [
                    'slug' => 'ppn-retur-penjualan',
                    'label' => 'PPN Retur Penjualan',
                    'nominal' => $data['order_tax_rate'] / 100 * $data['total_price'],
                    'cara_bayar' => $data['cara_bayar']
                ],
                [
                    'slug' => 'discount-retur-penjualan',
                ...
                [
                    'slug' => 'discount-retur-penjualan',
                    'label' => 'Total Cashback produk',
                    'nominal' => $data['total_cashback'],
                    'cara_bayar'       => $data['cara_bayar'],
                ],
                [
                    'slug' => 'penyesuaian-persediaan-retur-penjualan',
                    'label' => 'Penyesuaian Persediaan Retur Penjualan ' . ucfirst($data['cara_bayar']),
                    'nominal' => $data['total_price'] + ($data['order_tax_rate'] / 100 * $data['total_price']) - $data['total_discount'],
                    'cara_bayar' => null
                ],
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            $this->jurnal->hapus($lims_return_data->id, 'returns');
        } catch (\Exception $e) {
            \DB::rollback();
            \Log::error("return sale delete : {$e->getMessage()}");
