...
class GiftCardController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            'card_no' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'gift_cards')->where(function ($query) {
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->validate($request, [
            'card_no' => [
                'max:255',
                Rule::unique(env('TENANT_DB_CONNECTION').'.'.'gift_cards')->ignore($request['gift_card_id'])->where(function ($query) {
                    return $query->where('is_active', 1);
                }),
            ]
        ]);
