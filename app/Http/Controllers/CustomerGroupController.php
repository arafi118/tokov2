...
class CustomerGroupController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            'name' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'customer_groups')->where(function ($query) {
    ...
    public function update(Request $request, $id)
    {
        $this->validate($request, [
            'name' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'customer_groups')->ignore($request->customer_group_id)->where(function ($query) {
                    return $query->where('is_active', 1);
                }),
            ],
        ]);
