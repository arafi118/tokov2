...
class BrandController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            'title' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'brands')->where(function ($query) {
    ...
    public function update(Request $request, $id)
    {
        $this->validate($request, [
            'title' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'brands')->ignore($request->brand_id)->where(function ($query) {
                    return $query->where('is_active', 1);
                }),
            ],
