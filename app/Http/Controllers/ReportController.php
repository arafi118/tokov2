...
class ReportController extends Controller
{
    ...
    public function dailySaleObjectiveData(Request $request)
    {
        ...
        $totalData = DB::connection(env('TENANT_DB_CONNECTION'))->table('dso_alerts')
        ...
        if(empty($request->input('search.value'))) {
            $lims_dso_alert_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('dso_alerts')
                                  ->whereDate('created_at', '>=' , $starting_date)
                                  ->whereDate('created_at', '<=' , $ending_date)
                                  ->offset($start)
                                  ->limit($limit)
    ...
    public function productExpiry()
    {
        ...
        $lims_product_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('products')
    ...
    public function warehouseStock(Request $request)
    {
        ...
        if($role->hasPermissionTo('warehouse-stock-report')) {
            if(isset($request->warehouse_id))
                ...
                $warehouse_id = 0;
            if(!$warehouse_id) {
                $total_item = DB::connection(env('TENANT_DB_CONNECTION'))->table('product_warehouse')
                ...
                $total_price = DB::connection(env('TENANT_DB_CONNECTION'))->table('products')->where('is_active', true)->sum(DB::raw('price * qty'));
                $total_cost = DB::connection(env('TENANT_DB_CONNECTION'))->table('products')->where('is_active', true)->sum(DB::raw('cost * qty'));
            }
            else {
                $total_item = DB::connection(env('TENANT_DB_CONNECTION'))->table('product_warehouse')
                            ->join('products', 'product_warehouse.product_id', '=', 'products.id')
                ...
                $total_qty = DB::connection(env('TENANT_DB_CONNECTION'))->table('product_warehouse')
                                ->join('products', 'product_warehouse.product_id', '=', 'products.id')
                                ->where([
                                    ['products.is_active', true],
                                    ['product_warehouse.warehouse_id', $warehouse_id]
                                ])->sum('product_warehouse.qty');
                $total_price = DB::connection(env('TENANT_DB_CONNECTION'))->table('product_warehouse')
                                ->join('products', 'product_warehouse.product_id', '=', 'products.id')
                                ->where([
                                    ['products.is_active', true],
                                    ['product_warehouse.warehouse_id', $warehouse_id]
                                ])->sum(DB::raw('products.price * product_warehouse.qty'));
                $total_cost = DB::connection(env('TENANT_DB_CONNECTION'))->table('product_warehouse')
                                ->join('products', 'product_warehouse.product_id', '=', 'products.id')
                                ->where([
                                    ['products.is_active', true],
                                    ['product_warehouse.warehouse_id', $warehouse_id]
    ...
    public function bestSellerByWarehouse(Request $request)
    {
        ...
        while($start <= $end)
        {
            ...
            $best_selling_qty = DB::connection(env('TENANT_DB_CONNECTION'))->table('sales')
    ...
    public function profitLoss(Request $request)
    {
        ...
        $payment_recieved_number = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')->whereNotNull('sale_id')->whereDate('created_at', '>=' , $start_date)
            ->whereDate('created_at', '<=' , $end_date)->count();
        $payment_recieved = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')->whereNotNull('sale_id')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('payments.amount');
        $credit_card_payment_sale = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')
        ...
        $cheque_payment_sale = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')
                            ->where('paying_method', 'Cheque')
                            ->whereNotNull('payments.sale_id')
                            ->whereDate('payments.created_at', '>=' , $start_date)
                            ->whereDate('payments.created_at', '<=' , $end_date)->sum('payments.amount');
        $gift_card_payment_sale = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')
                            ->where('paying_method', 'Gift Card')
                            ->whereNotNull('sale_id')
                            ->whereDate('created_at', '>=' , $start_date)
                            ->whereDate('created_at', '<=' , $end_date)
                            ->sum('amount');
        $paypal_payment_sale = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')
                            ->where('paying_method', 'Paypal')
                            ->whereNotNull('sale_id')
                            ->whereDate('created_at', '>=' , $start_date)
                            ->whereDate('created_at', '<=' , $end_date)
                            ->sum('amount');
        $deposit_payment_sale = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')
        ...
        $payment_sent_number = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')->whereNotNull('purchase_id')->whereDate('created_at', '>=' , $start_date)
            ->whereDate('created_at', '<=' , $end_date)->count();
        $payment_sent = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')->whereNotNull('purchase_id')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('payments.amount');
        $credit_card_payment_purchase = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')
                            ->where('paying_method', 'Gift Card')
                            ->whereNotNull('payments.purchase_id')
                            ->whereDate('payments.created_at', '>=' , $start_date)
                            ->whereDate('payments.created_at', '<=' , $end_date)->sum('payments.amount');
        $cheque_payment_purchase = DB::connection(env('TENANT_DB_CONNECTION'))->table('payments')
                            ->where('paying_method', 'Cheque')
                            ->whereNotNull('payments.purchase_id')
                            ->whereDate('payments.created_at', '>=' , $start_date)
                            ->whereDate('payments.created_at', '<=' , $end_date)->sum('payments.amount');
            ...
            if($warehouse_id == 0) {
                    ...
                    foreach ($variant_id_all as $item_code => $variant_id) {
                        ...
                        if(count($lims_product_purchase_data)) {
                            foreach ($lims_product_purchase_data as $product_purchase) {
                                $unit = DB::connection(env('TENANT_DB_CONNECTION'))->table('units')->find($product_purchase->purchase_unit_id);
                                if($unit->operator == '*'){
                        ...
                        if(count($lims_product_transfer_data)) {
                            foreach ($lims_product_transfer_data as $product_transfer) {
                                $unit = DB::connection(env('TENANT_DB_CONNECTION'))->table('units')->find($product_transfer->purchase_unit_id);
                                if($unit->operator == '*'){
                        ...
                        if(count($lims_product_return_data)) {
                            foreach ($lims_product_return_data as $product_return) {
                                $unit = DB::connection(env('TENANT_DB_CONNECTION'))->table('units')->find($product_return->sale_unit_id);
                                if($unit->operator == '*'){
                        ...
                        if(count($lims_product_purchase_return_data)) {
                            foreach ($lims_product_purchase_return_data as $product_purchase_return) {
                                $unit = DB::connection(env('TENANT_DB_CONNECTION'))->table('units')->find($product_purchase_return->purchase_unit_id);
                                if($unit->operator == '*'){
                ...
                if($product->is_variant) {
                    ...
                    foreach ($variant_id_all as $item_code => $variant_id) {
                        ...
                        $nestedData['category'] = $product->category->name;
                        //purchase data
                        $nestedData['purchased_amount'] = DB::connection(env('TENANT_DB_CONNECTION'))->table('purchases')
                        ...
                        $lims_product_purchase_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('purchases')
                                    ->join('product_purchases', 'purchases.id', '=', 'product_purchases.purchase_id')->where([
                                        ['product_purchases.product_id', $product->id],
                                        ['product_purchases.variant_id', $variant_id],
                                        ['purchases.warehouse_id', $warehouse_id]
                        ...
                        $nestedData['purchased_qty'] = $purchased_qty;
                        //transfer data
                        /*$nestedData['transfered_amount'] = DB::connection(env('TENANT_DB_CONNECTION'))->table('transfers')
                                ...
                                ->where([
                                  ...
                                  ->sum('total');
                        $lims_product_transfer_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('transfers')
                                ->join('product_transfer', 'transfers.id', '=', 'product_transfer.transfer_id')
                                ->where([
                                    ['product_transfer.product_id', $product->id],
                                    ['product_transfer.variant_id', $variant_id],
                        ...
                        $nestedData['transfered_qty'] = $transfered_qty;*/
                        //sale data
                        $nestedData['sold_amount'] = DB::connection(env('TENANT_DB_CONNECTION'))->table('sales')
                        ...
                        $lims_product_sale_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('sales')
                                    ->join('product_sales', 'sales.id', '=', 'product_sales.sale_id')->where([
                                        ['product_sales.product_id', $product->id],
                                        ['variant_id', $variant_id],
                                        ['sales.warehouse_id', $warehouse_id]
                        ...
                        $nestedData['sold_qty'] = $sold_qty;
                        //return data
                        $nestedData['returned_amount'] = DB::connection(env('TENANT_DB_CONNECTION'))->table('returns')
                        ...
                        $lims_product_return_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('returns')
                                ->join('product_returns', 'returns.id', '=', 'product_returns.return_id')
                                ->where([
                                    ['product_returns.product_id', $product->id],
                                    ['product_returns.variant_id', $variant_id],
                        ...
                        $nestedData['returned_qty'] = $returned_qty;
                        //purchase return data
                        $nestedData['purchase_returned_amount'] = DB::connection(env('TENANT_DB_CONNECTION'))->table('return_purchases')
                                ...
                                ->where([
                                  ...
                                  ->sum('total');
                        $lims_product_purchase_return_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('return_purchases')
                                ->join('purchase_product_return', 'return_purchases.id', '=', 'purchase_product_return.return_id')
                                ->where([
                                    ['purchase_product_return.product_id', $product->id],
                                    ['purchase_product_return.variant_id', $variant_id],
    ...
    public function purchaseReport(Request $request)
    {
        ...
        foreach ($lims_product_all as $product) {
            ...
            if($warehouse_id == 0) {
                ...
                if($product->is_variant)
                    $variant_id_all = DB::connection(env('TENANT_DB_CONNECTION'))->table('purchases')
    ...
    public function saleReport(Request $request)
    {
        ...
        foreach ($lims_product_all as $product) {
            ...
            if($warehouse_id == 0){
                ...
                if($product->is_variant)
                    $variant_id_all = DB::connection(env('TENANT_DB_CONNECTION'))->table('sales')
    ...
    public function saleReportChart(Request $request)
    {
        ...
        foreach ($date_points as $key => $date_point) {
            $q = DB::connection(env('TENANT_DB_CONNECTION'))->table('sales')
                ->join('product_sales', 'sales.id', '=', 'product_sales.sale_id')
                ->whereDate('sales.created_at', '>=', $start_date)
