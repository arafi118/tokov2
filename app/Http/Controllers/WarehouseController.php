...
class WarehouseController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('backend.warehouse.create', compact('lims_warehouse_all'));
    }
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            'name' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'warehouses')->where(function ($query) {
    ...
    public function update(Request $request, $id)
    {
        $this->validate($request, [
            'name' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'warehouses')->ignore($request->warehouse_id)->where(function ($query) {
                    return $query->where('is_active', 1);
                }),
            ],
        ]);
