...
class ExpenseCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            'code' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'expense_categories')->where(function ($query) {
    ...
    public function update(Request $request, $id)
    {
        $this->validate($request, [
            'code' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'expense_categories')->ignore($request->expense_category_id)->where(function ($query) {
                    return $query->where('is_active', 1);
                }),
            ]
        ]);
