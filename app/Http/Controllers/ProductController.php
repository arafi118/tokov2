...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            'code' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'products')->where(function ($query) {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key => $sale)
            {
                ...
                if($sale->sale_unit_id) {
                    $unit_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('units')->select('unit_code')->find($sale->sale_unit_id);
    ...
    public function purchaseHistoryData(Request $request)
    {
        ...
        $q = DB::connection(env('TENANT_DB_CONNECTION'))->table('purchases')
        ...
        if(!empty($purchases))
        {
            foreach ($purchases as $key => $purchase)
            {
                ...
                if($purchase->purchase_unit_id) {
                    $unit_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('units')->select('unit_code')->find($purchase->purchase_unit_id);
                    $nestedData['qty'] .= ' '.$unit_data->unit_code;
                }
    ...
    public function saleReturnHistoryData(Request $request)
    {
        ...
        $q = DB::connection(env('TENANT_DB_CONNECTION'))->table('returns')
        ...
        if(!empty($returnss))
        {
            foreach ($returnss as $key => $returns)
            {
                ...
                if($returns->sale_unit_id) {
                    $unit_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('units')->select('unit_code')->find($returns->sale_unit_id);
                    $nestedData['qty'] .= ' '.$unit_data->unit_code;
                }
    ...
    public function purchaseReturnHistoryData(Request $request)
    {
        ...
        $q = DB::connection(env('TENANT_DB_CONNECTION'))->table('return_purchases')
        ...
        if(!empty($return_purchases))
        {
            foreach ($return_purchases as $key => $return_purchase)
            {
                ...
                if($return_purchase->purchase_unit_id) {
                    $unit_data = DB::connection(env('TENANT_DB_CONNECTION'))->table('units')->select('unit_code')->find($return_purchase->purchase_unit_id);
                    $nestedData['qty'] .= ' '.$unit_data->unit_code;
                }
    ...
    public function updateProduct(Request $request)
    {
        ...
        $this->validate($request, [
            'name' => [
                'max:255',
                Rule::unique(env('TENANT_DB_CONNECTION').'.'.'products')->ignore($request->input('id'))->where(function ($query) {
                    return $query->where('is_active', 1);
                }),
            ],
