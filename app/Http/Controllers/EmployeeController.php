    ...
    public function store(Request $request)
    {
        ...
        if(isset($data['user'])){
            $this->validate($request, [
                'name' => [
                    'max:255',
                        Rule::unique(env('TENANT_DB_CONNECTION').'.'.'users')->where(function ($query) {
        ...
        $this->validate($request, [
            'email' => [
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'employees')->where(function ($query) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if($lims_employee_data->user_id){
            $this->validate($request, [
                'name' => [
                    'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'users')->ignore($lims_employee_data->user_id)->where(function ($query) {
                        return $query->where('is_deleted', false);
                    }),
                ],
                'email' => [
        ...
        $this->validate($request, [
            'email' => [
                'email',
                'max:255',
                    Rule::unique(env('TENANT_DB_CONNECTION').'.'.'employees')->ignore($lims_employee_data->id)->where(function ($query) {
                    return $query->where('is_active', true);
                }),
            ],
            'image' => 'image|mimes:jpg,jpeg,png,gif|max:100000',
