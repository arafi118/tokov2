    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header mt-2">
                <h3 class="text-center">Arus Kas</h3>
            </div>
            @include('backend.laporan_keuangan.filter')
            <div class="row" style="margin:10px;">
            	<div class="col-md-12">
            		@if($tahun !='' && $bulan !='')
            		@include('backend.laporan_keuangan.arus_kas.table')
					@else
...
function doFilter(param=null){
	var is_param = '';
	var url ='{{url("laporan_keuangan/arus_kas?warehouse_id")}}='+$('#warehouse_id').val()+'&tahun='+$('#tahun').val()+'&bulan='+$('#bulan').val()+'&hari='+$('#hari').val();
